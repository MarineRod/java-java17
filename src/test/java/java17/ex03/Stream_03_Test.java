...
public class Stream_03_Test {
	...
	public void test_max() throws Exception {
		...
		Optional<Order> result = orders.stream()
                .max(Comparator.comparingDouble(order -> order.getPizzas().stream()
	...
	public void test_min() throws Exception {
		...
		//Optional<Order> result = null;
		Optional<Order> result = orders.stream()
                .min(Comparator.comparingDouble(order -> order.getPizzas().stream()
                                                               .mapToDouble(Pizza::getPrice) // Mappe chaque pizza à son prix
                                                               .sum())); // Calcule la somme des prix des pizzas pour chaque commande
	...
	public void test_map_collect_joining() throws Exception {
		...
		// par le caractère "|"
		//String result = null;
		String result = customers.stream().map(Customer::getFirstname).sorted() // Trie les prénoms par ordre alphabétique                
		.collect(Collectors.joining("|")); // Joins les prénoms avec le caractère "|"
	...
	public void test_flatMap() throws Exception {
		...
		//List<Pizza> result = null;
		 List<Pizza> result = orders.stream()
                 .flatMap(order -> order.getPizzas().stream()) // Aplatisse la liste des pizzas de chaque commande
                 .collect(Collectors.toList()); // Collecte toutes les pizzas dans une liste
